...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
	CSocketWrapperUDP::CSocketWrapperUDP(void)
#if defined(PLATFORM_WINDOWS)
...
#error UNSUPPORTED PLATFORM !
#endif
	{
		try
		{
			...
			if (WSAEventSelect(MSocket,MSocketEvent,FD_READ | FD_WRITE | FD_CLOSE)==SOCKET_ERROR)
			{
				int32											ErrorCode=CSocketError::SocketGetLastErrorCode();
				ESocketExceptionCode							SocketExceptionCode=CSocketError::GetSocketExceptionCode(ErrorCode);
...
#elif defined(PLATFORM_LINUX)
			MSocket=static_cast<int32>(socket(AF_INET,SOCK_DGRAM,0));
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::Uninitialize(void) noexcept
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
	CString CSocketWrapperUDP::ToString(void) const
	{
		return(CString::Format("UDP SOCKET WRAPPER [",GetSocketHandler(),"]"));
	}
//----------------------------------------------------------------------------------------------------------------------
	int32 CSocketWrapperUDP::GetSocketHandler(void) const noexcept
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	CSocketPlaformData CSocketWrapperUDP::GetPlatformData(void) const
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::BindToAddress(const CSocketUDPAddress& Address)
	{
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::SendTo(const CSocketUDPSendToParameters& Parameters, CSocketUDPSendToResult& OperationResult)
	{
		const CSocketUDPAddress&								Address=Parameters.GetUDPAddress();
		uint16													Port=Address.GetIPEndpoint().GetPort();
		...
		{
			throw(CSocketException("SOCKET FAILED to SEND DATA ! CONVERSION of ADDRESS to NUMERIC FORM FAILED !"));
		}

		int														Result=sendto(MSocket,Data,DataLength,0,reinterpret_cast<sockaddr*>(&SocketAddress),sizeof(SocketAddress));

		if (Result!=SOCKET_ERROR)
		{
			OperationResult.SetResult(Parameters.GetAddress(),Parameters.GetData(),Parameters.GetOffset(),static_cast<Size>(Result));
		}
		else
		{
			int32												ErrorCode=CSocketError::SocketGetLastErrorCode();
		...
		ssize_t													Result=sendto(MSocket,Data,DataLength,0,reinterpret_cast<sockaddr*>(&SocketAddress),static_cast<socklen_t>(sizeof(SocketAddress)));
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::ReceiveFrom(const CSocketUDPReceiveFromParameters& Parameters, CSocketUDPReceiveFromResult& OperationResult)
	{
		...
		int														Result=recvfrom(MSocket,Data,DataLength,0,reinterpret_cast<sockaddr*>(&SocketAddress),&SocketAddressLength);

		if (Result!=SOCKET_ERROR)
		{
			CSocketUDPAddress									Address(CSocketIPEndpoint(CSocketIPAddress(SocketAddress.sin_addr.S_un.S_un_b.s_b1,SocketAddress.sin_addr.S_un.S_un_b.s_b2,SocketAddress.sin_addr.S_un.S_un_b.s_b3,SocketAddress.sin_addr.S_un.S_un_b.s_b4),static_cast<uint16>(ntohs(SocketAddress.sin_port))));

			OperationResult.SetResult(Parameters.GetData(),Parameters.GetOffset(),static_cast<Size>(Result),Address);
		}
		else
		{
			int32												ErrorCode=CSocketError::SocketGetLastErrorCode();
			...
			if (ErrorCode==WSAEWOULDBLOCK)
			{
				OperationResult.SetResult(Parameters.GetData(),Parameters.GetOffset(),0,CSocketUDPAddress());
			}
			else
			{
		...
		ssize_t													Result=recvfrom(MSocket,Data,DataLength,0,reinterpret_cast<sockaddr*>(&SocketAddress),&SocketAddressLength);
		...
		if (Result!=-1)
		{
			...
			if (inet_ntop(AF_INET,&(SocketAddress.sin_addr),RawIPAddress,INET_ADDRSTRLEN)!=nullptr)
			{
				...
				{
					CSocketUDPAddress							Address(CSocketIPEndpoint(IPAddress,static_cast<uint16>(ntohs(SocketAddress.sin_port))));

					OperationResult.SetResult(Parameters.GetData(),Parameters.GetOffset(),static_cast<Size>(Result),Address);
				}
				else
				{
					throw(CSocketException("SOCKET FAILED to RECEIVE DATA !"));
				}
			}
			else
			{
				...
				throw(CSocketException("SOCKET FAILED to RECEIVE DATA ! CONVERSION of ADDRESS to STRING FAILED !",ErrorCode,SocketExceptionCode));
			}
		}
		else
		{
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::ChangeBlockingState(bool BlockingState)
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	int32 CSocketWrapperUDP::GetSendBufferSize(void)
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::SetSendBufferSize(int32 SendBufferSize)
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	int32 CSocketWrapperUDP::GetReceiveBufferSize(void)
	{
#if defined(PLATFORM_WINDOWS)
...
//----------------------------------------------------------------------------------------------------------------------
	void CSocketWrapperUDP::SetReceiveBufferSize(int32 ReceiveBufferSize)
	{
#if defined(PLATFORM_WINDOWS)
